...
import { GripVertical, Edit, Trash2, CalendarDays, Activity, CheckCircle, XCircle, MapPin } from 'lucide-react';
import { Button } from '@/Components/ui/button';

export function SortableCareerItem({ career, openEditModal, handleDelete, openActivityModal }) {
    ...
    return (
        <div ref={setNodeRef} style={style} className={`bg-card text-card-foreground border ${career.is_active ? 'border-border' : 'border-dashed border-border/50 opacity-80'} rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group flex flex-col relative`}>
            <div className="flex justify-between items-start mb-2 gap-2">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-lg leading-tight text-foreground">{career.job_title}</h3>
                        <p className="text-sm font-medium text-primary mt-0.5">{career.company_name}</p>
                    </div>
                </div>
            </div>

            <div className="pl-8 mb-4 flex flex-col gap-1.5">
                <span className="text-[11px] flex items-center text-muted-foreground">
                    <MapPin size={12} className="mr-1.5" /> {career.location}
                </span>
            ...
            <div className="mt-auto flex items-center justify-between border-t border-border pt-4 pl-8">
                <Button variant="outline" size="sm" className="h-8 text-xs bg-secondary/50" onClick={() => openActivityModal(career)}>
                    <Activity size={14} className="mr-2 text-primary" /> 
                    Kelola Aktivitas ({career.activities?.length || 0})
                </Button>
                <div className="flex gap-1">
                    <button onClick={() => openEditModal(career)} className="p-1.5 text-muted-foreground hover:text-primary transition-colors"><Edit size={16} /></button>
                    <button onClick={() => handleDelete(career.id)} className="p-1.5 text-muted-foreground hover:text-destructive transition-colors"><Trash2 size={16} /></button>
                </div>
            </div>
        </div>
    );
